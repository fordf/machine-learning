...
def predict(x, weights):
    ...
    actvs = forward_prop(x, weights)
    labels = actvs[-1].argmax(axis=1)
    return labels


def verbose_cost_gradients(nn_params, x, y, layer_sizes, reg_lambda):
    """Call cost_and_gradients, print cost."""
    cost, grads = cost_and_gradients(nn_params, x, y, layer_sizes, reg_lambda)
    print(cost)
    return cost, grads


def cost_and_gradients(nn_params, x, y, layer_sizes, reg_lambda):
    """Compute cost of dataset with nn_params."""
    m = x.shape[0]
    thetas = reroll(nn_params, layer_sizes)
    lb = LabelBinarizer()
    yvec = lb.fit_transform(y)
    actvs = forward_prop(x, thetas)
...
def forward_prop(x, thetas, return_activations=False):
...
def init_rand_weights(layer_sizes):
    params = []
...
def visualize(image_array, shape, labels=None, y=None, label_dict=None, order='F'):
    """Imshow each image_mtx alongside predicted label."""
    from matplotlib import pyplot as plt
    from random import sample
    image_array = [x.reshape(shape, order=order) for x in image_array]
